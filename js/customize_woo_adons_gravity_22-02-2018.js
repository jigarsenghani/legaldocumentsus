jQuery(document).ready(function ($) {
      ...
      $(document).on('click', '.datasorting', function(e){
        ...
        //el.find('.controllfilenameresponse').html('<img align="center" width="50px" height="50px" src="https://media.giphy.com/media/xTk9ZvMnbIiIew7IpW/giphy.gif">');
      ...
      $(document).on('click', '.rules_import_action', function(e){
		...
		$.ajax({
			...
			success: function (response) {
				...
				$( ".gravityform_inport_export_response" ).html( response);
       ...
       $( ".gravityform_inport_export_response" ).html( response.message); 
		...
		if(gfuser_settings  != 'traverse')
        {
          ...
          //$(this).closest(".gravityform-custom-div").find('.useraddcontrol_number').hide(); 
          $(".add_more_button").show();
        }
        ...
        type: "POST",
        url:  ajax_custom.ajaxurl,
        data: {action:action,userselectednamerules:userselectednamerules,controllername:controllername,controllernamerules:controllernamerules,productsku:productsku,rules_indexkey:rules_indexkey,gfuser_settings:gfuser_settings,useraddcontrol_number:bundle_data},
        success: function (response) {
          el.find('img:first-child').remove();
          //el.find('.controllfilenameresponse').append(response);
          el.find('.controllfilenameresponse').html();
          el.find('.controllfilenameresponse').html(response);
          ...
          //$(this).closest(".gravityform-custom-div").find('select').each(function(index){
            ...
            if ($(this).has('option:selected') && $(this).val() !== 'skip' ){
               gravity_Control.push([$(this).attr("name"), $(this).val(),$(this).data('gravity_filed_id')]);  
                ...
                //console.log(($(this).data('inputs_id')));
               gravity_ControlJson.push([$(this).attr("name"), $(this).val(),$(this).data('gravity_filed_id')]);
            }
          });
        ...
        function AjaxCall(fromID,eventType){
          ...
          var product_sku = $("#_sku").val();
		   var product_id = $("#post_ID").val();
            ...
            type: "POST",
            url:  ajax_custom.ajaxurl,
            data: {action:action,fromID:fromID,product_sku:product_sku,product_id:product_id},
            //dataType: 'json',
            success: function (response) {
